...
**Target instrument:** Symbol-agnostic — works on any instrument. EA input defaults are tuned for XAUUSD (gold), while the bundled `config.yaml` is tuned for BTCUSDT (crypto). Adjust SL/RR values per instrument in your `config.yaml`.  
